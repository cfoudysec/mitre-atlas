...
| 16 | [Lateral Movement](#-16-lateral-movement--amltao0015) | AML.TA0015 | Cat is moving from room to room |
...
### 🐾 Why It Matters
Pickle-based code execution in ML pipelines is a well-known vector — Trail of Bits has published research on it. If you load models from sources you don't fully trust, malicious serialization is a primary risk surface.
...
### 🐾 Real-World Echo
**SesameOp** (CS0042, added v5.2.0 January 2026) — adversaries used a legitimate AI assistant API as a covert command-and-control channel, blending malicious traffic into normal AI workflows.
...
### 🐾 Real-World Echo
### 🐾 Real-World Echo
**Morris II Worm** (CS0024) — RAG-based prompt injection self-propagates and exfiltrates PII without user interaction.
...
## 🐱 16. Lateral Movement — `AML.TA0015`

> *"The cat got into one room. Now the cat is in every room."*

### Description
Lateral Movement is the 16th ATLAS tactic, added in **v5.1.0 (November 2025)**. It captures the adversary's progression from initial AI-system access into adjacent systems, services, and identities — a tactic borrowed in concept from ATT&CK but with AI-specific paths that don't exist in traditional environments. Once an attacker has compromised an agent, an inference endpoint, or an MLOps platform, lateral movement happens through the agent's own legitimate access to downstream systems, through tool credentials harvested from agent configurations, through MCP servers and A2A peers the agent connects to, and through reused authentication material that propagates across services. AI-specific lateral movement is particularly difficult to detect because the traffic looks like authorized agent activity rather than classic intrusion behavior.

### Adversary Goal
Move from a compromised AI system into adjacent systems, accounts, or environments to expand the attack's reach.

### Notable Techniques
- **Use Alternate Authentication Material** (`AML.T0091`) — pivoting on stolen tokens, application access tokens
- **Cloud Service Discovery** in adjacent cloud environments
- **Agent Tool Pivoting** — using the agent's tool access to reach downstream services
- **MCP Server Pivoting** — moving across A2A or MCP boundaries
- **AI Service API as Pivot** — using legitimate AI APIs as a movement channel

### 🐾 Why It Matters
Adding Lateral Movement formalizes what defenders had been seeing in the wild: a compromised AI agent isn't a self-contained incident. It's a foothold. The case studies added alongside this tactic — including Slack AI exfiltration, Copilot Studio data pivots, and Jira Service Management injection — all involve the attacker using the agent's authorized access to reach systems they had no direct path to.
...
ATLAS publishes **42+ real-world case studies** as of v5.4.0 (February 2026). The table below lists case studies confirmed against the [official ATLAS changelog](https://github.com/mitre-atlas/atlas-data/blob/main/CHANGELOG.md) and [case study index](https://atlas.mitre.org/studies). Practitioner descriptions are summaries; consult the linked authoritative source for canonical content.

### 🆕 v5.4.0 — February 2026 Additions

| Case Study | ID | What It Demonstrates |
|---|---|---|
| **Exposed ClawdBot Control Interfaces Leads to Credential Access and Execution** | CS0048 | Misconfigured agent-control interfaces leading to credential access and execution |
| **Supply Chain Compromise via Poisoned ClawdBot Skill** | CS0049 | Poisoned skill introduced through an agent's plugin ecosystem |
| **OpenClaw 1-Click Remote Code Execution** | CS0050 | CSRF + agent configuration tampering chain (CVE-2026-25253) — link click leads to host compromise via agent sandbox escape |
| **OpenClaw Command & Control via Prompt Injection** | CS0051 | Indirect prompt injection abusing agent control tokens to invoke unrestricted execution tools |

### 🆕 v5.2.0 / v5.3.0 — January 2026 Additions

| Case Study | ID | What It Demonstrates |
|---|---|---|
| **SesameOp** | CS0042 | Novel backdoor using OpenAI Assistants API for command and control |
| **Malware Prototype with Embedded Prompt Injection** | CS0043 | Malware carrying prompt-injection payloads |
| **LAMEHUG** | CS0044 | Malware leveraging dynamic AI-generated commands at runtime |
| **Data Exfiltration via an MCP Server used by Cursor** | CS0045 | Data exfiltration through an MCP server connected to Cursor |
| **Data Destruction via Indirect Prompt Injection Targeting Claude Computer-Use** | CS0046 | Indirect prompt injection driving destructive actions via computer-use access |
| **Malicious AI Agent Discovered in Amazon Q VS Code Extension** | CS0047 | Malicious AI agent published as a VS Code extension |

### 🆕 v5.1.0 — November 2025 Additions

| Case Study | ID | What It Demonstrates |
|---|---|---|
| **Live Deepfake Image Injection to Evade Mobile KYC Verification** | CS0033 | Real-time deepfake injection defeating mobile KYC |
| **ProKYC: Deepfake Tool for Account Fraud Attacks** | CS0034 | Commodity deepfake tool used in account fraud |
| **Data Exfiltration from Slack AI via Indirect Prompt Injection** | CS0035 | RAG exfil from Slack AI via indirect injection |
| **AIKatz: Attacking LLM Desktop Applications** | CS0036 | LLM desktop application attacks |
| **Data Exfiltration via Agent Tools in Copilot Studio** | CS0037 | Exfiltration through legitimate agent tool invocations |
| **Planting Instructions for Delayed Automatic AI Agent Tool Invocation** | CS0038 | Dormant instructions executing on later invocations |
| **Living Off AI: Prompt Injection via Jira Service Management** | CS0039 | Prompt injection delivered through Jira tickets |
| **Hacking ChatGPT's Memories with Prompt Injection** | CS0040 | Persistent prompt injection planted in user memory |
| **Rules File Backdoor: Supply Chain Attack on AI Coding Assistants** | CS0041 | Supply chain attack via poisoned rules files |

### 📚 Earlier Foundational Case Studies (referenced in the official ATLAS site)

| Case Study | ID | What It Demonstrates |
|---|---|---|
| **Tay Poisoning** | — | Real-time data poisoning of a deployed chatbot through public input |
| **Morris II Worm: RAG-Based Attack** | CS0024 | RAG-based self-propagating prompt-injection worm |
| **PoisonGPT** | CS0019 | Lobotomized model published to a public hub bypassing safety benchmarks |
| **ChatGPT Package Hallucination** | CS0022 | Squatted package names registered after observing hallucination patterns |
| **Achieving Code Execution in MathGPT via Prompt Injection** | CS0016 | Prompt injection driving the application into RCE |
| **Camera Hijack Attack on Facial Recognition System** | CS0004 | Physical-layer attack on a deployed biometric system |
| **AI Model Tampering via Supply Chain Attack** | CS0027 | Production model swapped via dependency or pipeline compromise |
| **LLM Jacking** | CS0029/CS0030 | Stolen LLM API credentials reused or resold by attackers |
| **Google Bard Conversation Exfiltration** | CS0031 | Indirect prompt injection driving conversation exfil |
| **Financial Transaction Hijacking with M365 Copilot as an Insider** | CS0026 | Copilot used as an unwitting actor against financial workflows |
...
| An agentic system with tools | Execution, Privilege Escalation, C2, Lateral Movement |
...
| A multi-agent orchestration | Persistence, C2, Lateral Movement |
...
The October 2025 collaboration with **Zenity Labs** added 14 new agent-focused techniques. November 2025 (**v5.1.0**) added **Lateral Movement** (AML.TA0015) as the 16th tactic, plus 10 new techniques and 9 case studies (including the Hacking ChatGPT's Memories with Prompt Injection study). January 2026 (**v5.2.0** and **v5.3.0**) added 8 more techniques and the SesameOp, LAMEHUG, Cursor MCP Exfiltration, Claude Computer-Use Destruction, and Amazon Q VS Code Extension case studies. February 2026 (**v5.4.0**, the current latest) added agent-focused techniques **Publish Poisoned AI Agent Tool**, **Escape to Host**, **Exploitation for Credential Access**, **Exploitation for Defense Evasion**, **AI Agent**, plus the OpenClaw 1-Click RCE and OpenClaw Command & Control via Prompt Injection case studies. Community contributions drive real coverage.
...
- 🐾 [KryptoKat's Google SAIF 2.0 Reference](https://github.com/cfoudysec/kryptokat-google-saif)
